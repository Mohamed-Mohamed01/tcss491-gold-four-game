...
class KeyPickup {
  constructor(game, assetManager, camera, player, x, y, opts = {}) {
    ...
    // 
    // Support waking one OR many guards (backwards compatible)
    //new code 1:21
    this.guardEnemies = Array.isArray(opts.guardEnemies)
    ? opts.guardEnemies
    : (opts.guardEnemy ? [opts.guardEnemy] : []);

    this.wakeRadius = opts.wakeRadius ?? 180;
    this.guardWoken = false;
  ...
  update() {
    ...
    // 
    //new code 1:21
    if (this.guardEnemies.length > 0 && !this.guardWoken) {
      ...
      if (dk <= this.wakeRadius) {
        ...
        for (const enemy of this.guardEnemies) {
          if (enemy) enemy.asleep = false;
        }
      }
    }
